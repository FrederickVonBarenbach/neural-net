...
var rewards = [{pos: 0, reward: 100}, {pos: 10, reward: 1000}];
...
function beeProblem() {
  ...
  for (let n = 0; n < trainingIterations; n++) {
    ...
    for (let t = 0; t < timeLimit; t++) {
      ...
      if (qPair == undefined) {
        qPair = new QPair(bee.pos, Bee.actions.length);
        qTable.push(qPair);
      }
...
class Bee {
  ...
  static wait(bee, wordy = false) {
    if (wordy) console.log("waiting at pos:     " + bee.pos);
  }
  static goLeft(bee, wordy = false) {
    bee.pos -= 1;
    if (wordy) console.log("going left to pos:  " + bee.pos);
  }
  static goRight(bee, wordy = false) {
    bee.pos += 1;
    if (wordy) console.log("going right to pos: " + bee.pos);
  }
}
